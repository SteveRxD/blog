...
Summary: 'In this post I produce a population dot-density map using R, to better understand the relationship between Dallas, its neighboring cities, and something they call the "Metroplex".'
author: 'Steve Doogue'
date: '2018-05-19'
...
As a new arrival in Dallas, it's taken me a while to get to grips with its layout. Its sheer size and sprawl make it difficult to identify where Dallas ends and neighboring cities begin. Expanses of new developments and construction appear to have blurred the lines between what were once distinct towns and cities. 

With this in mind, this post has two aims: first, to better understand the geography of Dallas, and second, to do so while exploring some of the GIS tools available in R, the open-source programming language. Specially, I wanted to produce a [dot-density map](https://en.wikipedia.org/wiki/Dot_distribution_map) showing the approximate location of each person in Dallas and its surrounding areas.
...
To narrow things down, we can focus on four counties which account for over 85% of residents in the DFW Metroplex, namely Dallas County (2.6 million people), Tarrant County (2.0 million), Collin County (0.9 million), and Denton County (0.8 million). The map below shows these keys counties, overlaid by the boundaries of the City of Dallas.
...
When producing a dot-density map, we will allocate dots (representing people) to a random location within each block group. One problem is that these boundaries overlay bodies of water, including lakes, and we do not want to assign dots to areas that are clearly uninhabited. 

We can address this by 'erasing' the area of each block group that overlaps a major body of water. To do this we use the technique explained [here](https://walkerke.github.io/tidycensus/articles/spatial-data.html) by Kyle Walker, the author of the tidycensus package for R. The map below shows the same block group boundaries once the areas of water have been erased.
...
We are now ready to produce a dot-density map. Here we use one dot to represent 100 residents, and assign each dot to a random location somewhere in each block group. The results are shown below.
...
The map illustrates Dallas' sprawling population, and the absence of a clear delineation between neighboring cities and towns. For instance, when travelling north from downtown Dallas to McKinney (33 miles by road), or north west to Denton (40 miles), the density of the population appears to remain fairly constant. There are few blank areas on the map that would indicate rural or unoccupied land separating the cities.

The uninhabited spots on the map tend to be rivers and airports, often with abutting commercial and industrial zones. This includes the branches of the Trinity River running between Dallas and Lewisville, and between Dallas and Fort Worth. The DFW airport is a notable 'blank' spot in the map, lying to the north west of Irving (you can see these features by zooming in on the map).

__Population growth over time__

The final step is to produce a map showing how the population has _changed_ over time. Here we use dots to represent growth or reductions in the number of people. 
...
# GENERATE COORDINATE PAIRS FOR POPULATION GROWTH
#
# Creates a coordinate pair for each dot.
# Because this takes a very long time, the results are saved as a csv file.
# They can then be read as a csv file in future, without re-running this code.

# Data frame for population *growth*. Each dot = 50 people
# num_dots_growth <- as.data.frame(sf_blockgroup_change) %>%
#                     select(growth_pos,growth_neg) %>%
#                     mutate_all(funs(as.integer(round(. / 50))))
# 
# sf_dots_growth <-  map_df(names(num_dots_growth), 
#                       ~ st_sample(sf_blockgroup_change, size = num_dots_growth[, .x], type = "random") %>%
#                         st_cast("POINT") %>%
#                         st_coordinates() %>%
#                         as_tibble() %>% 
#                         setNames(c("lon","lat")) %>%
...
# Write to csv
# write_csv(sf_dots_growth,'data/sf_dots_growth.csv')
...
**Fig 5: Population growth between 2010 and 2016 (1 dot = 50 people)**
```{r, echo = FALSE, message = FALSE, warning = FALSE}
...
This map shows a general decline in population levels near the centers of Dallas and Fort Worth. There is, however, a dense cluster of growth near downtown Dallas. Most of the growth is concentrated in the outlying towns and cities. In particular, the map highlights the rapid growth seen to the north, particularly in Frisco and McKinney, which in 2016 were the [second- and third-fastest growing large cities in the United States.](https://www.census.gov/newsroom/press-releases/2017/cb17-81-population-estimates-subcounty.html)
...
And that's it! Two dot-density maps showing the distribution of Dallas' population and its change over time, all produced using R. This illustrates R's GIS capabilities, and the ability to work with spatial features (such as maps and boundary files) alongside regular data sets.  
<br>  
{{<alert info>}} 
__Technical stuff__  
  
The code used to produced this post is available on [GitHub](https://github.com/SteveRxD/Blog/tree/master/content/post).

Most of the analysis takes advantage of the [tidycensus](https://walkerke.github.io/tidycensus/) package. This enables the user to interact with data from the US Census Bureau’s decennial census and American Community Survey APIs, including the ability to download related spatial information (in this case, block groups and water features).  
  
The simple features or [sf](https://cran.r-project.org/package=sf) package allows you to store spatial information as columns in a data frame, which which can then be manipulated like any other data frame; for example, by using dplyr operators and functions such as `%>%`, `filter` and `left_join`.  
  
Finally, the [leaflet](https://rstudio.github.io/leaflet/) package for R was used to produce the interactive maps shown above.
{{</alert>}}
