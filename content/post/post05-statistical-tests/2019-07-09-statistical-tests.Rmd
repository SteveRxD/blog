...
thumbnailImage: //res.cloudinary.com/ddjmrpbw4/image/upload/c_thumb,w_200,g_face/v1563202273/05%20Statistical%20tests/swissarmy.png
comments: true
...
The results our independent sample t-test, of whether there are statistically significant differences between men's and women's salaries, are _exactly the same_ as our test for whether $\beta_1$ is different from zero! Both tests give the same p-value, which, if sufficiently low (typically below 5%) would lead us to reject the null hypothesis that there was no difference in salaries.

The book shows how the linear model can be applied in a wide range of other cases, making it the 'Swiss Army knife' of statistical tests. It also demonstrates that many non-parametric tests, which are needed when certain test assumptions do not hold, can be approximated by linear models using the _rank_ of values. 

This approach brings coherence to a wide range of statistical tests, which are usually taught to students as independent tools with a 'zoology' of names.  The approach also helps to explain the intuition underlying statistical tests (rather than relying on rote learning) by drawing on the familiar concept of linear regressions.

As well as serving to solidify my own understanding, I hope this will be helpful to others following along with Lindeløv's original book. My version of the book was written using the [bookdown](https://bookdown.org/) package in R. The GitHub repository can be found [here](https://github.com/SteveRxD/Stat_tests).
