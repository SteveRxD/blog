...
Summary: 'Building a simple app to access public crime data and map the location of each incident in my neighorhood'
author: ''
date: '2019-01-15'
slug: 
categories: []
...
As can be seen, the app allows the user select the neighborhoods they're interested in, and then displays incidents occuring in those neighborhoods over the selected timeframe (from  the past month to past three years).
...
4. **Create shapefiles of my local neighbourhoods**. I wanted to identify incidents that occured in specific neighbourhoods, as opposed to the wider ZIP codes. I could not find these boundary maps online (in the form of shapefiles), so I drew them manually in [Google MyMaps](https://www.google.com/maps/about/mymaps/). These boundaries could be downloaded as .kml files and converted to shapefiles [here](https://mygeodata.cloud/converter/kml-to-shp). The shapefiles were then read into R using the `readOGR()` function.
