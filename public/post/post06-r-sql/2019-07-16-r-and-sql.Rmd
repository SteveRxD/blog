...
slug: r-and-sql
tags: []
thumbnailImage: //res.cloudinary.com/ddjmrpbw4/image/upload/c_thumb,w_200,g_face/v1563498573/06%20R%20SQL/r_to_sql_thumb1.png
thumbnailImagePosition: left
Summary: Many common SQL queries can also be performed with R, and vice versa. This post uses examples to demonstrate similarities between the two languages. Part 1 looks at selecting, sorting and filtering data.
---


_When analyzing data, many common procedures in SQL can also be performed in R, and vice versa. Here I demonstrate the similarities between the two. For people with a basic knowledge of either R or SQL, this may make the process of learning the other language more intuitive. In Part 1, we look at selecting, sorting and filtering data. In Part 2 (to follow) we will look at joining data._
...
-   [**3. Filtering**](#part3)
 ...
 -  [**4. Joins**](#part4)
    -  [**4.1  Inner join**](#part4_1)
    -  [**4.2  Left join**](#part4_2)
<hr>
...
- __orders__: sales orders placed by customers, including the order and shipping date
...
1. Type queries straight into the browser using the [MySQL TryIt](http://www.mysqltutorial.org/tryit/) tool; or
...
When running the code in R, we will create a data frame that corresponds to each table in the relational database. These tables can be downloaded as individual .csv files [here]() [need to add link]. Load the data in R using the following code:
...
To get data from a table in SQL we use the `SELECT` statement, and `FROM` to specify the table. If we want _all_ records from a table, we use `SELECT *`. For example, to see all the records on the employees table we would use:
...
A similar approach can be used when you want to see distinct combinations of two or more variables. 
...
For example, if you wanted to return the five customers with the greatest credit limits you could use the following:
...
Filtering is used to restrict our results to rows that satisfy one or more criteria. We already saw some basic filtering in the above, in which we used SQL's `WHERE` clause and R's `filter()` to only shows cities that were within the United States. 
...
A couple of differences with R:  
...
This shows us the three customers who are both located in California and have credit limits over $100,000:  
...
Say we want to list every customer from the `customers` table with an associated order in the `orders` table. We want to see each customer number, customer name, order number(s) and order status. We will order the results by customer name.

This is an exampe of an innter join. To do this in SQL we would use `INNER JOIN... ON`, and in R we would use the `inner_join()` function. 
...
Note that in the code above we need to specify which table each column comes from, e.g. using `customers.customerNumber` instead of just `customers`. To make the code more readable, we can shorten it using [aliases](http://www.mysqltutorial.org/mysql-alias/). This assigns an alternative name to a table or column, using the `AS` keyword, which can then be used in place of the full name. To illustrate, the following code gives the exact same result as the one above:
...
Note that the `AS` keyword is optional, and could have been excluded from the query above. 

_With R:_

The same join could be carried out in R as follows:
...
This codes returns the following 326 rows:
```{r, echo = FALSE}
customers %>% 
  inner_join(orders, by = "customerNumber") %>% 
  select(customerNumber, customerName, orderNumber, status) %>% 
